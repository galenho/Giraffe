...
local ConnectServer = require "network.connect_server"
...
function Master.fun_console_callback(cmd)
	...
	elseif cmd == "show server" then
		global.connect_server:ShowServer()
	end
end
...
function Master:Run()
	...
	global.connect_server = ConnectServer:New()
	global.connect_server:Init()
	global.connect_server:Start()
